...
def create_post():
    data = request.json  # Getting the JSON data from the request
    title = data.get('title')  # Extracting title from the data
    content = data.get('content')  # Extracting content from the data
    category_id = data.get('category_id')  # Extracting category ID from the data
    user_id = data.get('user_id')  # Extracting user ID from the data

    if not title or not content or not category_id:  # Checking if all required fields are present
        return jsonify({'error': 'Title, content, and category are required'}), 400  # Returning an error message if any field is missing
    ...
    try:
        cursor.execute(
            ...
            (title, content, user_id, category_id)  # Inserting the post data into the database
        )
        connection.commit()
    ...
    return jsonify({'message': 'Post created successfully'})  # Returning a success message

@app.route('/categories', methods=['POST'])  # Route to create a new category
def create_category():
    data = request.json  # Getting the JSON data from the request
    name = data.get('name')  # Extracting name from the data
    description = data.get('description')  # Extracting description from the data

    if not name or not description:  # Checking if all required fields are present
        return jsonify({'error': 'Name and description are required'}), 400  # Returning an error message if any field is missing
    ...
    return jsonify({'message': 'Category created successfully'})  # Returning a success message
...
def like_post(post_id):
    ...
    return jsonify({'message': 'Post liked successfully'})  # Returning a success message
...
def get_user(user_id):
    ...
    if user:  # If user exists
        ...
        return jsonify({'error': 'User not found'}), 404  # Returning an error message if user does not exist

@app.route('/users/<int:user_id>', methods=['DELETE'])  # Route to delete a user by ID
def delete_user(user_id):
    connection = get_db_connection()
    cursor = connection.cursor()
    try:
        cursor.execute('DELETE FROM users WHERE user_id = %s', (user_id,))  # Query to delete the user
        connection.commit()
    ...
    return jsonify({'message': 'User deleted successfully'})  # Returning a success message
